...
이번 과제에서도 사용자 입력 오류 시 재입력을 받아야한다.
저번에는 모든 검증을 한 클래스에서 모두 진행함으로써 그 문제를 해결했지만
이번에는 객체들의 역할을 명확하게 하기 위해 그렇게 하지 않고 각 객체가 알아야 하는 정보만 검증하고,
올바르지 않은 값일 경우 예외를 발생시켰다.

## 객체의 역할

사용자 입력 정보가 로직에서 사용되는 절차를 나열해봤다.
...
이 모든 절차를 도메인, View, Controller, domain, Util 등 어떤 패키지에서 어떤 클래스가 동작해야 하는지 생각해봤다.

**1. 문자열 -> 기본입력 검증 (빈 문자열, 숫자가 아닌 경우 등)**

입력이 뷰에서 들어오기 때문에 아주 기본적인 검증은 view - InputView 의 역할이라고 생각했다.

**2. 기본 검증된 문자열 -> 입력 형식에 올바른지 검증**

입력 형식이라는 것은 비즈니스 로직과는 무관하다고 볼 수 있고,
비즈니스 로직은 그대로지만 입력 형식을 바꾸고 싶을때,
도메인 영역을 건들지 않고 수정할 수 있게 하기위해 util - InputParser 의 역할로 했다.

**3. 입력 형식 검증된 문자열 -> 문자열과 정수**

이것도 마찬가지 입력 형식을 구조화 하는것은 util 패키지 - InputParser의 역할이다.

**4. 문자열과 정수 -> Menu 인스턴스와 정수**

이 부분은 문자열을 Menu 라는 도메인 객체로 만드므로 컨트롤러 또는 서비스 계층의 역할이다.
서비스 계층은 따로 둬도 되지만, 도메인 로직을 직접적으로 접근하므로 domain - MenuService 에게 맡기기로 했다.

**5. Menu 인스턴스와 정수 -> OrderedMenu 객체**
...
확실히 각 요소 마다 복잡한 함수를 다르게 적용해야할 경우에는 Enum 보다는 class 를 사용하는 것이 좋은것 같다.
확장성도 좋고 코드 가독성도 좋아졌다.
...
이번 과제에서 두 데이터를 묶어서 사용하기 위해 Map 컬렉션을 많이 사용했다.
그런데 사용하다 보니 Map 은 특정 데이터 접근이 빠른건 맞지만 전체 컬렉션을 순회하는데에는 좋지 않은 컬렉션이라는 생각이 들었다.
방법을 고민하다가 결국 모든 EnumMap 과 Map 을 dto를 따로 만들어서 사용하기로 했다.
자바의 **record** 를 사용했다. 필드의 불변성을 보장하고 게 와 생성자를 자동으로 제공하기 떄문에 코드가 간단하고 dto 사용에 최적화이다.

덕분에 기존 구현 코드를 매우 많이 바꿨지만 Map.entry 를 사용하지 않게됨으로써 로직은 간단해졌다.

### 인터페이스에 대해

인터페이스를 추상클래스가 받아서 이벤트 클래스들이 그것을 구현하도록 했었는데, 큰 의미 없이 복잡도만 늘리는거 같아서 추상클래스로 통합했다.
구현할 때 인터페이스에 뭘 적고 추상클래스에 뭘 적고 하느라 복잡했는데 합치니 훨씬 로직이 간결해졌다.
굳이 필요하지 않은 클래스 구조를 늘리는 것은 의미없는것 같다.

### 기능 확장

2023년 12월 뿐만이 아니라 2024년 12월, 더 나아가 무슨 날짜이든지 간에 Application.java 에서 숫자만 바꿔주면 작동하도록 하고싶었다.

구현해보다가 결국 직접 만들어서 사용하던 Date 클래스의 한계를 느끼고 그냥 LocalDate 를 사용하기로 했다.

자바 기본 API 를 진작에 사용할 걸 그랬다. 몇년 몇월의 마지막 날짜는 며칠인지, 무슨 요일인지 등 많은 기능을 제공하고 있었다.

코드를 아주 많이 변경하긴 했지만 2064년 11월을 입력해도 작동하도록 개선되었다.
